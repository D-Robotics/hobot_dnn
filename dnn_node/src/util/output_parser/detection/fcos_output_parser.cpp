...
void CqatGetBboxAndScoresScaleNHWC(std::vector<std::shared_ptr<DNNTensor>> &tensors,
  ...
  for (int i = 0; i < 5; i++) {
    ...
    bool czxdebug = true;
    std::cout << "czxdebug i = " << i << std::endl;
    std::cout << "ce_scale[0]: " << ce_scale[0] << std::endl; 
    ...
    for (int h = 0; h < tensor_h; h++) {
      for (int w = 0; w < tensor_w; w++) {
        ...
        if (czxdebug) {
          std::cout << "h: " << h << " w: " << w << " ce_data: " << ce_data[ce_offset] << " ce_data_offset: " << ce_data_offset << " cls_data_offset: " << cls_data_offset << " score: " << score << std::endl;
        }
        ...
        std::cout << "xmin: " << detection.bbox.xmin << " ymin: " << detection.bbox.ymin << " xmax: " << detection.bbox.xmax << " ymax: " << detection.bbox.ymax << " score: " << detection.score << std::endl;
...
int PostProcess(std::vector<std::shared_ptr<DNNTensor>> &tensors,
  ...
  std::cout << "post tensors.size(): " << tensors.size() << std::endl;
  ...
  if (community_qat_) {
    std::cout << "community_qat_" << std::endl;
